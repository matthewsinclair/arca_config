...
- AC-00.1 (non-test) No public symbol is retired without the sibling-fleet probe (`function_exported?` / `Code.ensure_loaded?` / direct refs) re-run at removal time and recorded -- the `register_change_callback/2` tripwire (arca_cli liveness proxy) survives or arca_cli is migrated first -- evidence: impl.md removal log + vc ack below -- satisfied: yes
  - **vc ACK (2026-08-04), verified mechanically rather than read.** The removal log claims no public symbol was retired in the entire thread. I did not take that on trust: I extracted every public `def` from `lib/` at the pre-ST0002 baseline `9925115` and at `03969fa` and diffed the sets. **85 public defs before, 95 after. Zero modules removed**; three added (`Arca.Config.CLI`, `Arca.Config.Error`, `Arca.Config.Value`).
  - **One delta, adjudicated rather than waved through.** Exactly one public `def` present at baseline is absent now: `Server.handle_info({:notify_paths, _}, _)` (`server.ex:394` at `9925115`). It is a GenServer callback -- invoked by OTP, never callable by a consumer -- so it is not a public symbol in the sense this AC protects. Three checks before accepting that: the message `{:notify_paths, _}` has **zero hits across arca_config and arca_cli**, so sender and handler went together and it was only ever server-to-self; the capability it implemented (per-key delivery of `{:config_updated, path, value}`) is live and covered at `server_test.exs:562,582,613` and `notification_matrix_test.exs:97`; and its removal is WP-03's deliberate move from asynchronous self-messaging to notification before the call returns, which removes a race rather than adding one.
  - **The named tripwire survives on both sides.** `register_change_callback/2` is live at `arca_config.ex:447` and `server.ex:256`, and arca_cli's liveness proxy still probes exactly that function at `arca_cli.ex:129`. arca_cli did not need migrating first.
  - **Coverage of this ack**: I checked the public `def` surface of `lib/` and the module set. I did **not** re-run the sibling-fleet probe myself -- that evidence is cc's, recorded at removal time -- so I ack it as recorded rather than as reproduced.
- AC-00.2 (non-test) arca_cli rebuilt against the final arca_config (mix.lock advanced from 8b30615) and its full suite passes, executed by vc -- evidence: vc report at `intent/st/ST0002/vc-rebuild-report.md` -- satisfied: yes
  - Executed by vc under hv's release control. Lock advanced `8b30615` -> `5db55a4`. arca_cli `952667d`: **782 passed** (29 doctests, 753 tests) across seeds 1/3/11/77/555/4242, `--warnings-as-errors` and `--check-formatted` clean, ST0011 contract 57/57, escript rebuilt and probed end-to-end. One MED filed in the report (arca_cli re-swallows enoent for a *configured* path); it fails no test, is strictly better than the 0.4.x behaviour it replaces, and its fix is identified.
...
### WP-05 -- Surface and dependency pruning (status: DONE 2026-08-04, all six ACs satisfied)
...
- AC-06.1 (non-test) vc's rebuild of arca_cli against the final arca_config passes its full suite; failures triaged to migration notes or fixed here -- evidence: vc report cited in impl.md -- satisfied: yes
  - Report: `intent/st/ST0002/vc-rebuild-report.md`. Both rebuild failures were triaged and **fixed in arca_cli** (WP-15, `dadcb07`), not deferred to migration notes: the R1 tuple reached users raw at two seams, `setting_error/2` and `reason_text/1`. The second was not in the handover's prediction -- `reason_text/1` was a silent duplicate of `ErrorHandler.format_reason/1` and was deleted rather than patched.
...
- AC-06.3 (non-test) CHANGELOG + version bump per ruling R5 + tag by hv -- evidence: CHANGELOG.md written and version bumped to 0.3.0 in mix.exs and README; annotated tag `v0.3.0` created at hv's instruction 2026-08-04 and pushed to upstream, pointing at `ccd8fb5` -- the tree is identical to the CI-green build `03969fa`, everything between being documentation -- satisfied: yes
- AC-06.4 (non-test) `usage-rules.md` (or successor consumer doc) carries actual arca_config API guidance for consumer LLM sessions -- evidence: rewritten 2026-08-04 (AF-39). It now leads with a consumer DO/NEVER contract -- mandatory setup, the error shape and how to match it, writes that can fail, the precedence trap that outranks the generic variables, `get_config_location/0` for diagnosis, the notification matrix, and testing guidance -- with the Intent tooling content kept below for contributors. The `register_change_callback/2` tripwire is stated in NEVER DO -- satisfied: yes
